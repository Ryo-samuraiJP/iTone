...
// DisplayAlbum component that displays the album details and songs based on the album id
const DisplayAlbum = () => {
  const { id } = useParams();
  ...
  const totalDurationInMinutes = Math.floor(totalDurationInSeconds / 60); // Convert the total duration to minutes
  const hours = Math.floor(totalDurationInMinutes / 60); // Get the total hours
  const minutes = totalDurationInMinutes % 60; // Get the total minutes

  // Truncate the text based on the screen width
  const truncateText = (text) => {
    if (window.innerWidth <= 640) {
      // Truncate the text to 25 characters if the screen width is less than or equal to 640px
    ...
    <>
      ...
      <div className="mt-10 flex gap-8 flex-col md:flex-row md:items-end">
        ...
        <div className="flex flex-col">
          <p>Playlist</p>
          <h2 className="text-5xl font-bold mb-6 md:text-7xl">
            {albumData.name}
          </h2>
        ...
        >
          <div className="text-white flex items-center">
            {track.id === item.id && playStatus ? (
              ...
              <b className="mr-4 text-[#a7a7a7] font-normal w-14 md:w-7 xl:w-6 text-right">
                {index + 1}
              </b>
            )}
            <img
              ...
              src={item.image}
              alt="each track image"
            />
            <span className="flex flex-col w-full">
              <span className="truncate">{truncateText(item.name)}</span>
              <span className="block text-sm text-[#a7a7a7] truncate">
                {item.artist}
              </span>
            </span>
          </div>
